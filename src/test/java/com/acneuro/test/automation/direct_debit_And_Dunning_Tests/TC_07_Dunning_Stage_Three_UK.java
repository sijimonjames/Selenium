...
public class TC_07_Dunning_Stage_Three_UK {
	...
	private static void dunningStageOne() throws SQLException {
		...
		try {
			...
			File localDunningStageOneFile = FileUtilsLibrary.csvFileForDunningRejection(diseAccountNumber,
					customerNumber, "STDDS1", "DMV1", "126", country);
	...
	private static void dunningStageThree() {

		try {
			// Dunning stage 1 file
			File localDunningStagethreeFile = FileUtilsLibrary.csvFileForDunningRejection(diseAccountNumber,
					customerNumber, "DISSUB", "DMTV", "201", country);
			// Move the file from local directory to Remote location
			FileTransferLibrary.transferFileToRemoteDirectory(Constant.JACKAL_HOSTNAME, localDunningStagethreeFile,
					remoteDir_Payment_Rejection);
			// Run the dunning mediation job
			// Run the worker
			MVNO_DunningWorkflowSpecific.runWorkerDuuningMediation_Jackal();
