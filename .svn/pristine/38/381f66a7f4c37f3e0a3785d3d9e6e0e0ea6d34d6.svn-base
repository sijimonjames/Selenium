 ...
 *  @Test: Reinstate of DD after Dunning stage 1 for NON-Fatal DD rejection
 */

public class TC_02_DD_Restore_Dunning_Stage_One_FR {
	...
	public void CC_Retry_Tests() {
		...
		try {
			...
			//Check for Payment method is DD as DD is NON-FATAL
			disePaymentType = MVNO_DunningWorkflowSpecific.getAccountDetails(diseAccountNumber, "PaymentType",
					Country.FR.name());
	...
	private static void dunningStageOne() throws SQLException {
		...
		try {
			...
			File localDunningStageOneFile = FileUtilsLibrary.csvFileForDunningRejection(diseAccountNumber,
					customerNumber, "STDDS1", "DMV1", "151", country);
	...
	private static void Repayment_dunningStageOne() {
